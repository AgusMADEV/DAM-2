Para la asignatura de Programación multimedia y de dispositivos, he decidido escoger mi proyecto **`Minecraft Isométrico`**, en el que he utilizado Three.js como motor de renderizado 3D. Es un proyecto muy ambicioso, y que tiene mucho margen de mejora, pero que con lo que tiene ahora mismo, cumple con creces, lo pedido en clase. 
Es un proyecto de un juego sandbox (mundo abierto) en el que el jugador puede **explorar, destruir, construir y recolectar objetos**, y todo en un entorno 3D generado proceduralmente.

Este proyecto sirve para demostrar que he comprendido los conceptos fundamentales de los motores de juegos: 
...
El juego está diseñado para que funcione en los navegadores web sin necesidad de instalaciones adicionales, utilizando las tecnologías web estándar (HTML5, CSS3, JavaScript ES6+).

---

### La arquitectura del Motor del Juego

El juego implementa una arquitectura modular que se suele ver en motores de juegos modernos:

-**Motor de renderizado**: Sistema que transforma la geometría 3D en una imagen 2D en pantalla (Three.js + WebGL)
-**GameLoop**: El bucle principal que se encarga de actualizar la lógica del juego y renderiza los frames.
-**Raycasting**: Es una técnica para detectar qué objeto se encuentra debajo del cursor del ratón (en este caso para saber donde apunta el jugador).
...
const CONFIG = {
      WORLD_SIZE: 32,           // Tamaño del mundo (32x32 bloques) - AMPLIADO
      ...
      TERRAIN_SCALE: 0.3,       // Escala del ruido para terreno (más bajo = más suave)
...
let cameraAngle = 45;         // Ángulo inicial de la cámara
...
// Geometrías reutilizables (optimización)
let blockGeometry;
let blockMaterials = {};
...
### Funcionamiento:
Ahora vamos a ver el funcionamiento paso a paso del proyecto:
...
function init() {
    ...
    scene.fog = new THREE.Fog(0x87CEEB, 40, 80); // Ajustado para mundo más grande
    ...
    // Crear geometrías y materiales reutilizables
...
**Paso 2: Generación procedural del terreno**:
...
Creamos el funcionamiento de las **físicas y el movimiento del personaje** (punto muy clave):
```js
function updatePlayer(deltaTime) {
    ...
    // Aplicar movimiento con detección de colisiones (SIN escalado automático)
    ...
    // Saltar - 1 bloque de altura solamente
    if (keys.space) {
    ...
    // Actualizar indicador de capa (altura del suelo donde está el personaje)
...
function checkCollision(x, y, z) {
    // Verificar si hay un bloque sólido bloqueando el movimiento
    const blockX = Math.floor(x);
    const blockZ = Math.floor(z);
    
    // Obtener la altura del suelo actual donde está el personaje
    const currentGroundHeight = getGroundHeight(player.x, player.z);
    
    // Obtener la altura del suelo en la posición objetivo
    ...
    // Si el suelo objetivo está más de 1 bloque arriba, no puede pasar sin saltar
    if (heightDifference > 1) {
    return true; // Bloqueado: demasiado alto
    }
    
    // Si el suelo objetivo está 1 bloque arriba y NO está saltando, bloquear
    ...
    // Verificar si hay un bloque justo donde estarían los pies
    ...
    return false; // No hay colisión, puede moverse
}
```

**Paso 4: Sistema de items e inventario**:
Creamos diferentes funciones para: (crear un item, actualizarlo a cuando esta flotando en el mundo, recogerlo del mundo y para acabar actualizamos la UI de inventario):

```js
function createItem(x, y, z, type) {
    // Crear un cubo pequeño flotante para el item
    ...
    type: type,
    createdAt: Date.now(),
    floatOffset: Math.random() * Math.PI * 2 // Para animación flotante única
    };
...
function updateItems() {
    ...
    const time = Date.now() * 0.001; // Tiempo en segundos para animación
    ...
    // Recoger si está cerca (1.5 bloques de distancia)
    if (distance < 1.5) {
        collectItem(item, i);
    }
    }
...
**Paso 5: Game Loop**:
Aquí creamos el bucle principar para actualizar la lógica  y renderizar los frames del juego:
...
function animate() {
    ...
    // Actualizar items (animación y recolección)
    updateItems();
...
### Terminología técnica que hemos aplicado
...
### Aplicación e implementación del proyecto
El proyecto está totalmente funcional con los siguientes sistemas que he implementado:
...
function onMouseDown(event) {
    ...
    // Verificar distancia al personaje (rango de interacción)
    // Usar la posición del centro del bloque
    ...
    // Click derecho: colocar
        ...
        // Calcular posición del nuevo bloque basado en la cara
...
```js
//Reutilizar geometría para ahorrar memoria
...
**Materiales compartidos causanban highlight en todos los bloques del mismo material**: 
...
**Verificación de colisiones inexacta**: 
...
### Optimizaciones aplicadas en el juego
...
<head>
  ...
  <title>Minecraft Isométrico - Proyecto Final</title>
  <style>
    * {
      margin: 0;
      padding: 0;
...
<body>
  ...
  <div id="welcome-screen">
    <div class="welcome-content">
      <h1>🎮 Minecraft Isométrico</h1>
      <p>Proyecto Final - Análisis de Motores de Juegos</p>
      <p>Controla a tu personaje y construye en un mundo 3D con vista isométrica</p>
      <p><strong>WASD</strong> para mover • <strong>Espacio</strong> para saltar • <strong>Click</strong> para construir/destruir</p>
  ...
  <div id="ui-panel">
    ...
    <div class="control"><span class="key">Q/E</span> - Rotar cámara</div>
  </div>
  ...
  <div id="stats">
    ...
    <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.2);"><strong>🎲 Seed:</strong> <span id="world-seed">0</span></div>
  </div>
  ...
  <div id="inventory" style="position: fixed; top: 220px; right: 20px; background: rgba(0,0,0,0.7); padding: 15px; border-radius: 10px; color: white; font-family: 'Segoe UI', sans-serif; font-size: 14px; min-width: 150px;">
    <h3 style="margin: 0 0 10px 0; font-size: 16px;">📦 Inventario</h3>
  ...
  <script>
    // ============================================================================
    // ZONA DE PROPIEDADES DE INICIO (Configuración inicial del juego)
    // ============================================================================
    ...
    let cameraDistance = CONFIG.CAMERA_DISTANCE;
    let gameStarted = false;
    let worldSeed = Math.random() * 10000; // Semilla aleatoria para generación del mundo
    ...
    function setupLights() {
      ...
      // Ajustar cámara de sombras para mundo 32x32
      directionalLight.shadow.camera.left = -40;
      directionalLight.shadow.camera.right = 40;
      directionalLight.shadow.camera.top = 40;
      directionalLight.shadow.camera.bottom = -40;
    ...
    function createPlayer() {
      ...
      // Posición inicial cerca del centro, con un offset aleatorio pequeño
      const spawnX = CONFIG.WORLD_SIZE / 2 + (Math.random() - 0.5) * 4;
      const spawnZ = CONFIG.WORLD_SIZE / 2 + (Math.random() - 0.5) * 4;
      
      player = {
        x: spawnX,
        y: 10,  // Altura inicial (se ajustará con gravedad)
        z: spawnZ,
    ...
    // ============================================================================
    // GENERACIÓN DEL TERRENO INICIAL
    // ============================================================================
    
    // Función de ruido simplificado con semilla (Perlin noise básico)
    function noise2D(x, z) {
      // Función de ruido pseudo-aleatoria pero suave usando la semilla del mundo
    ...
    function generateTerrain() {
      ...
      for (let x = 0; x < CONFIG.WORLD_SIZE; x++) {
        ...
        for (let z = 0; z < CONFIG.WORLD_SIZE; z++) {
          // Obtener altura base usando ruido Perlin en múltiples octavas
          let height = 0;
          let amplitude = 1;
          let frequency = 1;
          
          // 4 octavas para más detalle
          ...
          // Crear variación de biomas con otro ruido
          const biomeNoise = getPerlinNoise(x * 0.05, z * 0.05);
          
          // Zonas de montaña (biomeNoise > 0.6)
          if (biomeNoise > 0.6) {
            // Montañas: alturas de 4-8 bloques
            height = Math.pow(height, 0.7) * 8; // Exponente para hacer picos más pronunciados
          } 
      ...
      for (let x = 0; x < CONFIG.WORLD_SIZE; x++) {
        for (let z = 0; z < CONFIG.WORLD_SIZE; z++) {
          ...
          // Generar capas del terreno con lógica realista
    ...
    function removeBlock(block) {
      ...
      if (index > -1) {
        // Crear item en la posición del bloque destruido
    ...
    function onMouseMove(event) {
      ...
      // Aplicar hover al nuevo bloque solo si está en rango
      if (intersects.length > 0) {
        ...
        // Solo iluminar si está dentro del rango de interacción (4.5 bloques, similar a Minecraft)
        if (distance <= 4.5) {
    ...
    // ============================================================================
    // FÍSICA Y MOVIMIENTO DEL PERSONAJE
    // ============================================================================
    ...
    function getGroundHeight(x, z) {
      ...
      // Buscar el bloque más alto en esa posición
      for (let y = 10; y >= 0; y--) {
...
He construido un pequeño motor, jugable, que integra **rednerizado 3D**, **física simple**, **raycasting**, **generación procedural** y **UI in‑game**.
