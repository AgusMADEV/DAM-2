El proyecto **NuevaAgenda** consiste en una aplicación de gestión de citas médicas que implementa un **componente de acceso a datos basado en ficheros JSON**.  
A diferencia del enfoque tradicional con bases de datos SQL, este proyecto demuestra que es posible **persistir, consultar y modificar información estructurada** mediante ficheros, aprovechando el formato **JSON** como almacén principal.

El objetivo principal es crear una aplicación funcional y visualmente moderna que permita **crear, actualizar, cancelar, eliminar y exportar citas**, garantizando persistencia y consistencia sin depender de un servidor de base de datos.  
El contexto de uso es una **agenda local**, pensada para consultorios o profesionales que necesitan registrar citas y estadísticas de manera offline.

---

### Arquitectura general
El proyecto sigue una **estructura modular**, separando responsabilidades por capas:

| Archivo | Función principal |
|----------|------------------|
| `domain.py` | Define la entidad `Appointment` (modelo de datos) y su lógica de dominio. |
| `storage.py` | Implementa el repositorio `JsonAppointmentRepo`, responsable de leer y escribir en ficheros JSON. |
| `services.py` | Contiene la capa de servicios (`AgendaService`) que aplica validaciones, reglas de negocio y estadísticas. |
| `validators.py` | Provee funciones de validación (`v_fecha`, `v_hora`, `v_telefono`). |
| `app.py` | Capa de presentación (interfaz Tkinter). Gestiona el flujo entre el usuario y los servicios. |

El **fichero `appointments.json`** almacena las citas de forma estructurada, asegurando persistencia entre sesiones.

### Componentes técnicos

#### A) Modelo de datos (`domain.py`)
La clase `Appointment` utiliza `@dataclass` para definir campos tipados y automatizar la serialización.  
Incluye métodos para cambiar estado, registrar historial, reprogramar citas y convertir la información a diccionarios listos para JSON.
...
```
Cada cita tiene su propio identificador único (`uuid`), fecha y hora, y mantiene trazabilidad de cambios mediante `historial` y `notas_seguimiento`.

#### B) Capa de persistencia (`storage.py`)
El repositorio `JsonAppointmentRepo` gestiona la lectura y escritura de los datos en un fichero JSON dentro del directorio `data/`.  
Esto sustituye el uso de SQL mediante **operaciones CRUD directas sobre un archivo estructurado**.

**Ejemplo del proyecto:**
```python
class JsonAppointmentRepo:
    def __init__(self, path='data/appointments.json'):
        self.path = Path(path)
...
```
Este enfoque permite persistencia **sin motor de base de datos**, manteniendo el formato legible y exportable.

#### C) Capa de servicios (`services.py`)
`AgendaService` implementa la lógica de negocio: creación, validación, reprogramación, cancelación y generación de estadísticas.  
El método `validar()` asegura coherencia de fechas y horarios, y evita solapamientos.
...
def crear(self, a: Appointment):
    ...
    return a
```
Además, calcula métricas como número de citas atendidas o tiempo medio de espera.

#### D) Interfaz gráfica (`app.py`)
La aplicación usa **Tkinter** con `ttk.Style` para aplicar un diseño moderno, ofreciendo interacción completa con la capa de servicios.  
El usuario puede agregar, filtrar, exportar e importar citas sin interactuar directamente con el fichero JSON.
...
```
Este método crea un nuevo objeto `Appointment`, lo valida y lo guarda usando `AgendaService` → `JsonAppointmentRepo`.

#### E) Validadores (`validators.py`)
Garantizan que las entradas sigan formatos estándar:
...
def v_fecha(s: str) -> bool:
    try:
        ...
        return False
```
De esta forma, el sistema evita errores de formato en la entrada de datos.

---

###  Uso real
Al ejecutar `app.py`, el usuario accede a una interfaz que permite:
...
- Añadir nuevas citas con fecha, hora y descripción.
- Editar o cancelar citas existentes.
- Exportar e importar datos a ficheros JSON o CSV.
- Visualizar estadísticas mediante gráficos integrados (Tkinter + Canvas).

**Ejemplo de persistencia:**  
Cuando el usuario crea una cita, se serializa como:
```json
{
  "fecha": "15/12/2025",
  "hora": "12:00",
...
```
El componente `JsonAppointmentRepo` guarda este registro dentro de `data/appointments.json`, manteniendo todos los campos estructurados.

### Errores comunes y cómo evitarlos
1. **Formato incorrecto de fecha/hora:** se valida con `v_fecha()` y `v_hora()` antes de crear la cita.  
2. **Duplicidad de citas:** el servicio `AgendaService` evita crear citas en el mismo día y hora.  
3. **Edición sin selección:** el método `_actualizar_cita()` muestra advertencias si no se selecciona ninguna cita.  
4. **Fichero JSON corrupto:** el repositorio crea uno nuevo vacío si no existe, previniendo fallos de lectura.  
5. **Exportación interrumpida:** se maneja con `try/except` mostrando mensajes de error en ventana.

---

El proyecto **NuevaAgenda** demuestra que la persistencia de datos puede lograrse **sin SQL**, utilizando ficheros JSON y una arquitectura modular.  
Se aplican los principios de la unidad: **abstracción del acceso a datos, validación, separación por capas y manejo de formatos estructurados**.

Conecta directamente con otros contenidos vistos, como la manipulación de ficheros, serialización, validación de datos y diseño de servicios.  
El resultado es un sistema completo, funcional y ampliable que ejemplifica el **espíritu del acceso a datos sin bases de datos relacionales**.