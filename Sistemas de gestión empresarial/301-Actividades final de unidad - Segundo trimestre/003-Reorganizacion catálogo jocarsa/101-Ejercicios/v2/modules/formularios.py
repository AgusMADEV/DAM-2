...
def execute(context):
    ...
    if action == "create_formulario":
        ...
        for form in data["formularios"]:
            if form["id"] == formulario_id:
                if "titulo" in params:
                    form["titulo"] = params["titulo"]
                if "descripcion" in params:
                    form["descripcion"] = params["descripcion"]
                if "activo" in params:
                    form["activo"] = params["activo"]
                
                _save_data(context, data)
                return {"formulario": form, "message": "Formulario actualizado"}
        
        return {"error": "Formulario no encontrado"}
    
    elif action == "delete_formulario":
        # Eliminar formulario y sus respuestas
        formulario_id = params.get("id")
        
        for i, form in enumerate(data["formularios"]):
            if form["id"] == formulario_id:
                # Eliminar respuestas relacionadas
                data["respuestas"] = [r for r in data["respuestas"] if r.get("formulario_id") != formulario_id]
                
                # Eliminar formulario
                deleted = data["formularios"].pop(i)
                _save_data(context, data)
                return {"formulario": deleted, "message": "Formulario y sus respuestas eliminados"}
